...
const quickConfirmButton =
  "//button[@class='QuickOrder_confirmButton__1bNYl btn btn-primary disabled']";
...
When(
  ...
  async function () {
    ...
    const quickorderlocator = await page.locator(sucessMessage);
    // assert that it's visible
    await expect(quickorderlocator).toBeVisible();
    //
    await page.click(quickAddButton);
    const quickAddlocator = await page.locator(sucessMessage);
    // assert that it's visible
    await expect(quickAddlocator).toBeVisible();
    //
    await page.click(quickConfirmButton);
    const quickconfirmlocator = await page.locator(sucessMessage);
    // assert that it's visible
    await expect(quickconfirmlocator).toBeVisible();
    //
